...
def fetch_all(query, params=()):
    """Always use a fresh cursor so we never get stale cached results."""
    cursor = mydb.cursor()
...
def login_page():
    ...
    if st.button("Login"):
        if username == "superadmin" and password == "admin@123":
            ...
            # Fresh cursor + reconnect to avoid stale cache from other sessions
            mydb.reconnect()
            user = fetch_all(
            ...
            if user:
                user = user[0]
                st.session_state.update(logged_in=True, is_superadmin=False, user=user, branch_id=user[3])
                st.rerun()
            else:
...
def section_customer_sales(branch_id, is_superadmin):
    ...
    filter_start_date = filter_cols[date_start_idx].date_input("Start Date", value=None, key="sf_start_date")
    filter_end_date   = filter_cols[date_end_idx].date_input("End Date",   value=None, key="sf_end_date")

    products       = get_distinct("product_name", "customer_sales", branch_id=branch_id, is_superadmin=is_superadmin)
    filter_product = filter_cols[prod_idx].selectbox("Product", ["All Products"] + products, key="sf_product")
    ...
    if clicked_branch is not None:
        name_row = fetch_all("SELECT branch_name FROM branches WHERE branch_id = %s", (clicked_branch,))
        ...
        selected_drill_product = st.selectbox(
            "Filter by Product", ["All Products"] + branch_product_list, key="drill_product_select"
        )
    ...
    if st.session_state.action == "view_sales":
        ...
        if st.button("Submit Sale"):
            if not all([entry_branch_id, name, mobile_number, product_name, gross_sales]):
                ...
                """, (next_sale_id, entry_branch_id, date, name, mobile_number, product_name, gross_sales))
...
def section_payment_splits(branch_id, is_superadmin):
    ...
    filter_sale = filter_cols[sid_idx].selectbox("Sale ID", ["All Sales"] + sid_list, key="pf_sale")
    ...
    if st.session_state.action == "view_payment":
        ...
        sale_id_input  = st.text_input("Sale ID", placeholder="Enter the Sale ID to apply this payment to")
        ...
        if st.button("Submit Payment Split"):
            if not all([sale_id_input, amount_paid, payment_method]):
                st.warning("Please fill all fields.")
            else:
                try:
                    sale_id_val = int(sale_id_input)
                    exists = fetch_all("SELECT sale_id FROM customer_sales WHERE sale_id = %s", (sale_id_val,))
                    if not exists:
                        st.error(f"Sale ID {sale_id_val} does not exist. Please enter a valid Sale ID.")
                    else:
                        execute_commit(
                            "INSERT INTO payment_splits (payment_id, sale_id, payment_date, amount_paid, payment_method) VALUES (%s, %s, %s, %s, %s)",
                            (next_payment_id, sale_id_val, payment_date, amount_paid, payment_method)
                        )
                        st.success("Payment split submitted successfully!")
                        st.session_state.action = None
                except ValueError:
                    st.error("Sale ID must be a valid number.")
...
def section_branch_details(branch_id, is_superadmin):
    ...
    if st.session_state.action == "view_branch":
        ...
        if data:
            df = pd.DataFrame(data, columns=["Branch ID", "Branch Name", "branch_admin_name"])
            st.dataframe(df, use_container_width=True)
        else:
...
def section_users(branch_id, is_superadmin):
    ...
    if st.session_state.action == "view_users":
        # Reconnect to ensure we see users registered in other sessions
        mydb.reconnect()
        if is_superadmin:
        ...
        if data:
            df = pd.DataFrame(data, columns=["User ID", "Username", "Password", "Branch ID", "Role", "email"])
            st.dataframe(df, use_container_width=True)
        else:
